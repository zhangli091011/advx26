...
import { HomeAssistantManager } from "@/lib/home-assistant";
...
class PitHub extends EventEmitter {
  ...
  private homeAssistant: HomeAssistantManager | null = null;
  ...
  start() {
    ...
    try {
      this.homeAssistant = new HomeAssistantManager(config.homeAssistant);
      this.homeAssistant.start(
        (channel) => {
          ...
          if (channel.online) {
            this.state.connection.deviceLastSeen.homeAssistant = channel.updatedAt || Date.now();
          }
          this.markUpdated(Date.now());
        },
        (connected, timestamp) => {
          this.state.connection.deviceLastSeen.homeAssistant = connected ? timestamp : null;
          this.markUpdated(timestamp);
        },
      );
    } catch (error) {
      console.error(
        `[home-assistant] 配置加载失败：${error instanceof Error ? error.message : String(error)}`,
      );
  ...
  async setHomeAssistantPower(id: string, on: boolean) {
    if (!this.homeAssistant) throw new Error("Home Assistant 未配置");
